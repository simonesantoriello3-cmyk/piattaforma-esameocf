...
export default function Navbar() {
  ...
  return (
    <nav className="bg-blue-50 border-b border-blue-100 px-6 py-4 relative">
            ...
            <>
              ...
              <div className="relative hidden md:flex items-center gap-2" ref={dropdownRef}>
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-blue-100 transition-colors"
                >
                  ...
                  <span className="text-blue-800 text-sm font-medium">Ciao, {nomeUtente} 👋</span>
                  <span className="text-blue-500 text-xs ml-1">{dropdownAperto ? '▲' : '▼'}</span>
                </button>
                <button
                  onClick={logout}
                ...
                {dropdownAperto && (
                  <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-2xl shadow-lg border border-gray-100 z-50 overflow-hidden">
                    <div className="px-4 py-3 border-b border-gray-100">
                      <p className="text-blue-900 font-semibold text-sm">{nomeUtente}</p>
                      <p className="text-blue-500 text-xs">Account attivo</p>
                    </div>
                    <Link
                      href="/profilo"
                    ...
                    >
                      <span>👤</span>
                      <span className="text-blue-900 font-semibold text-sm">Profilo</span>
                    </Link>
                    <Link
                      href="/dashboard"
                      onClick={() => setDropdownAperto(false)}
                    ...
                    >
                      <span>📊</span>
                      <span className="text-blue-900 font-semibold text-sm">Dashboard</span>
                    </Link>
              ...
              <button
                ...
                aria-label="Menu"
              >
                <span className={`block w-6 h-0.5 bg-blue-800 transition-all duration-200 ${menuAperto ? 'rotate-45 translate-y-2' : ''}`}></span>
                <span className={`block w-6 h-0.5 bg-blue-800 transition-all duration-200 ${menuAperto ? 'opacity-0' : ''}`}></span>
                <span className={`block w-6 h-0.5 bg-blue-800 transition-all duration-200 ${menuAperto ? '-rotate-45 -translate-y-2' : ''}`}></span>
              </button>
            </>
          ) : (
            <>
              <Link href="/login" className="text-blue-800 hover:text-blue-900 text-sm font-medium md:hidden">
                Accedi
              </Link>
              <Link href="/login" className="text-blue-800 hover:text-blue-900 text-sm font-medium hidden md:inline">
                Accedi
              </Link>
              <Link href="/registrazione" className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors hidden md:inline-flex">
                Registrati
              </Link>
            </>
          )}
      ...
      {user && menuAperto && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-white border-b border-blue-100 shadow-lg z-50">
          <div className="flex flex-col px-6 py-5 gap-1">
            <div className="flex items-center gap-3 pb-4 mb-2 border-b border-blue-100">
            ...
            <Link
              ...
              className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-blue-50 transition-colors"
            >
